...
function getColor(discipline) {
    if (contains(discipline.label, doneDisciplines))
        return '#cce3f5';
    else if (contains(discipline.label, doingDisciplines))
        return '#0083b9';
    else
        return '#b3b3b3';
}

function contains(discipline, list) {
    for (let i = 0; i < list.length; i++)
        if (discipline === list[i])
            return true;
    return false;
}
